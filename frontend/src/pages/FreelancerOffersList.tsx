...
export function FreelancerOffersList() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['freelancerOffers', statusFilter, page],
    queryFn: () =>
      offerService.getFreelancerOffers({
  ...
  const declineMutation = useMutation({
    mutationFn: offerService.declineOffer,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['freelancerOffers'] });
    },
  });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-text mb-2">Offers Received</h1>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <p className="text-sm text-text-muted text-center max-w-sm mb-6">
            {statusFilter
              ...
              : 'You haven\'t received any offers yet. Keep applying to jobs to receive offers from clients.'}
          </p>
        </div>
      ) : (
        <>
