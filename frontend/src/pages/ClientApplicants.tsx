...
export function ClientApplicants() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['allClientApplicants', params],
    queryFn: () => clientApplicationService.getAllApplicants(params),
  });
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto p-4 lg:p-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-heading font-bold text-text mb-2">All Applicants</h1>
          <p className="text-text-muted">
            {total > 0 ? `${total} applicant${total !== 1 ? 's' : ''} across all your jobs` : 'No applicants yet'}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <EmptyApplicationsState
            title={search || statusFilter ? 'No applicants found' : 'No applicants yet'}
            description={
              search || statusFilter
